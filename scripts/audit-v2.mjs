...
if (process.argv.includes("--a11y-only"))
  sizes.push(
    ...JSON.parse(await readFile("artifacts/v2/viewports.json", "utf8")),
  );
else
...
await writeFile("artifacts/v2/viewports.json", JSON.stringify(sizes, null, 2));
...
for (const [width, theme] of [
  ...
  await page.setViewportSize({ width, height: 900 });
  await page.evaluate((t) => localStorage.setItem("fvf:theme", t), theme);
  for (const path of routes) {
    await page.goto(base + path, { waitUntil: "networkidle" });
...
await writeFile("artifacts/v2/viewports.json", JSON.stringify(sizes, null, 2));
await writeFile(
  "artifacts/v2/accessibility.json",
