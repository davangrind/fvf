...
for (const width of [1440, 390]) {
  ...
  for (const [name, path] of [
    ...
    await page.screenshot({
      path: `artifacts/v3/${name}-${width}.png`,
      fullPage: name !== "docs",
    });
    await page.screenshot({
      path: `artifacts/v3/${name}-${width}-viewport.png`,
    });
  ...
  await page.screenshot({
    path: `artifacts/v3/home-dark-${width}.png`,
    fullPage: true,
  });
  await context.close();
}
await writeFile(
  "artifacts/v3/inspection.json",
